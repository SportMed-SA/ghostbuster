...
Ghostbuster is a simple command-line tool that helps you keep translation files clean by finding keys nobody uses anymore. It scans nested JSON translations, checks your frontend source code for referenced keys, and reports what is safe to clean up.
...
Find translation keys that exist in JSON files but are not referenced in source files. Ideal for audits before release.
...
1. Run `detect` to review unused keys.
